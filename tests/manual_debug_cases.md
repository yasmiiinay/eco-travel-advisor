...
| B-4 | **budget** step → type `65`, then (new trip) `100`, then `180`; also tap the **Mid** and **Comfort** buttons | 65→Budget €, 100→Mid €€, 180→Comfort €€€. Chat tier name and summary label are identical (`€100/day · Mid €€`). No "Premium". (One mapping: backend `BUDGET_LABELS` + frontend `BUDGET_LABEL` both Budget/Mid/Comfort. If the old labels still show, hard-refresh — `index.html` now cache-busts `app.js?v=3`.) |
...
| B-5b | **date** step → type nonsense `asdkfj qwerty next bluemoon` (or `bluemoon`, `65`, random words) | Rejected; travel_date **not** set: "I couldn't recognise that as a travel date. Please choose a date range or select \"Flexible dates\"." The date controls re-appear. |
| B-5c | **date** step → tap **Flexible dates**, or send a UI range `2026-07-02 to 2026-07-09` / `15 August to 20 August` | Accepted: "Flexible dates" or the range string is stored and the form moves on. Safe phrases like `next week` are also accepted. |
